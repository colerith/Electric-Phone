  ...
  <div class="moments-view">
    ...
    <template v-else-if="panel === 'home'">
      ...
      <div class="moments-me-menu">
        ...
        ><button type="button" @click="panel = 'own'">
          <i class="fa-regular fa-images"></i><span>我的朋友圈</span
          ><small>{{ phone.state.moments.posts.length }} 条</small></button
        ><button type="button" @click="panel = 'wallet'">
          <i class="fa-solid fa-wallet"></i><span>我的钱包</span><i class="fa-solid fa-chevron-right"></i></button
        ><button type="button" @click="panel = 'settings'">
          <i class="fa-regular fa-comments"></i><span>朋友圈互动</span><i class="fa-solid fa-chevron-right"></i>
        </button>
      </div>
    </template>
    ...
    <template v-else-if="panel === 'settings'">
      ...
      <section class="settings-card system-settings-card moments-form">
        <div class="wave-settings-title">自动互动</div>
        <div class="system-toggle-row">
          <span>跟随聊天自动生成</span
          ><WaveToggle v-model="settings.followEnabled" aria-label="朋友圈跟随聊天自动生成" />
        </div>
        ...
        <div class="system-toggle-row">
          ...
          ><WaveToggle v-model="settings.npcEnabled" aria-label="允许场景 NPC 参与朋友圈" />
        </div>
      ...
      <section class="settings-card system-settings-card moments-form">
        ...
        <div class="settings-slider-row">
          ...
          ><WaveSlider v-model="settings.postProbability" :min="0" :max="100" :step="5" aria-label="朋友圈发帖概率" />
        </div>
        <div class="settings-slider-row">
          <span
        ...
        <div class="settings-slider-row">
          ...
          ><WaveSlider v-model="settings.likeProbability" :min="0" :max="100" :step="5" aria-label="朋友圈点赞概率" />
        </div>
      ...
      <section class="settings-card system-settings-card wave-clear-section">
        <div class="wave-settings-title">内容管理</div>
        <p>清空所有朋友圈动态及其点赞、评论，并从后续生成参考中移除。</p>
        <button v-if="!clearConfirm" type="button" class="wave-clear-button" @click="clearConfirm = true">
          <i class="fa-regular fa-trash-can"></i>一键清空朋友圈
        </button>
    ...
    <div v-if="!viewingNpc && (view === 'feed' || panel === 'own')" class="moments-feed">
        ...
        <div class="moment-post-main">
          ...
          <div v-if="post.images.length" class="moment-media-grid" :class="{ single: post.images.length === 1 }">
            <button v-for="(media, index) in post.images" :key="index" type="button" @click="preview = media">
              <img v-if="media.kind === 'image'" :src="media.url" :alt="media.description || '朋友圈图片'" /><span
                v-else
          ...
          <div class="moment-meta">
            ...
            ><button
              type="button"
              :aria-pressed="phone.state.moments.likes.includes(post.id)"
      ...
      <div v-if="!posts.length" class="messenger-empty">
        {{ view === 'me' ? '还没有发布朋友圈' : '还没有动态，发布第一条生活记录吧' }}
      </div>
    </div>
    ...
    <Teleport v-if="surface && composing" :to="surface">
      ...
      >
        <div class="moment-compose-bar">
          ...
          ><strong>{{
            selection === 'mentions' ? '提醒谁看' : selection === 'audience' ? '选择可见联系人' : '发布朋友圈'
          }}</strong
    ...
    <WaveImageUpload
      ...
      :label="imageTarget === 'cover' ? '朋友圈封面' : imageTarget === 'post' ? '朋友圈图片' : '我的头像'"
      @cancel="imageTarget = ''"
      @confirm="applyImage"
...
import WaveWalletWorkspace from './WaveWalletWorkspace.vue';
import WaveBilingualSettings from './WaveBilingualSettings.vue';
...
const props = defineProps<{ view: 'feed' | 'me'; userName: string; userAvatar: string }>();
defineEmits<{ share: [post: MomentPost, author: string] }>();
const phone = usePhoneStore(),
  surface = inject(phoneSurfaceKey, ref(null));
const viewingNpc = ref('');
const failedAvatars = ref(new Set<string>());
const panel = ref<'home' | 'profile' | 'settings' | 'own' | 'wallet'>('home'),
  notice = ref('');
...
const posts = computed(() =>
  phone.momentsFeed.posts.filter(
    post => post.availableAt <= now.value && (props.view === 'feed' || post.authorKey === 'user'),
  ),
);
...
function publish() {
  try {
    ...
    closeComposer();
    now.value = Date.now();
    if (props.view === 'me') panel.value = 'own';
  } catch (error) {
...
const subpageTitle = computed(() =>
  viewingNpc.value
    ? '详细资料'
    : { home: '我的', profile: '编辑资料', settings: '朋友圈互动', own: '我的朋友圈', wallet: '我的钱包' }[panel.value],
);
