<template>
  <section ref="root" class="messenger-app" :class="{ 'is-moments': tab === 'activity' }">
...
import { formatMessagePreview } from '../services/message-format';
import WaveMoments from './WaveMoments.vue';
import { groupContacts } from '../services/contact-alphabet';
...
type Tab = 'messages' | 'contacts' | 'activity' | 'me';
...
const tabs: { id: Tab; name: string; icon: string }[] = [
  ...
  { id: 'activity', name: '动态', icon: 'fa-regular fa-compass' },
...
const momentsSubpage = computed(() => (tab.value === 'me' || tab.value === 'activity') && !!moments.value?.isSubpage);
const headerTitle = computed(() =>
  momentsSubpage.value
    ? moments.value?.subpageTitle || '我的'
    : { messages: '消息', contacts: '联系人', activity: '朋友圈', me: '我的' }[tab.value],
);
const headerIcon = computed(() =>
  momentsSubpage.value && tab.value === 'activity'
    ? ''
    : tab.value === 'activity'
      ? 'fa-solid fa-camera'
      : tab.value === 'me'
        ? moments.value?.canPublish
          ? 'fa-solid fa-camera'
          : ''
        : 'fa-solid fa-plus',
);
const headerLabel = computed(() =>
  tab.value === 'activity' ? '发布朋友圈' : tab.value === 'me' ? '发布朋友圈' : '新增聊天或联系人',
);
