...
const moduleSettings = read('components/settings/WaveModuleSettings.vue');
const momentsView = read('components/space/WaveMoments.vue');
...
assert.match(
  space,
  /\.space-comment-avatar\s*\{[\s\S]*?width: 30px;[\s\S]*?height: 30px;[\s\S]*?flex: 0 0 30px;[\s\S]*?clip-path: circle\(50% at 50% 50%\);[\s\S]*?border-radius: 50% !important;/,
);
assert.match(
  space,
  /\.space-comment-avatar img\s*\{[\s\S]*?position: absolute;[\s\S]*?width: 100% !important;[\s\S]*?height: 100% !important;[\s\S]*?object-fit: cover !important;[\s\S]*?border-radius: 0 !important;/,
);
...
assert.match(
  messenger,
  /\.messenger-avatar img\s*\{[\s\S]*?object-position: center;[\s\S]*?border-radius: 0 !important;/,
);
...
assert.match(moduleSettings, /props\.app === 'calendar' \|\| props\.app === 'zone' \? null/);
assert.equal((momentsView.match(/<WaveBilingualSettings/g) || []).length, 1);
assert.match(momentsView, /Object\.assign\(phone\.settings\.moduleSettings\.zone,/);

console.log(
  'PASS: profile, moments, comments, messenger and character-setting avatars use stable circular crops; manual Moments generation is labeled Space.',
);
